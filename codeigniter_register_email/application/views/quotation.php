...
<div id="myModal" class="modal fade" role="dialog" tabindex="-1" style="width: 50%;height: 100vh;margin-left: 50%;background-color: green">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form style="color: #000;" method="post" action="<?php echo base_url('user/edit_quotation'); ?>">
          <div class="container">
            <div class="col-md-12 form-group">
          <label>Add  Quotation Prefix</label>
    ...
    <div class="form-row container">
     ...
     if ($fetch_clients->num_rows()>0) {
      ...
      ?></option>
                                ...
                                if($fetch_clientcompany->num_rows()>0){
                                  foreach ($fetch_clientcompany->result() as $row) {
                                   echo "<option value='" .$value->id . "'>" . $row->name . "</option"; 
                                  }
                                }
                                ?>
                                </option>
                     ...
                     <option><?php 
                                if($fetch_clients->num_rows()>0){
                                  foreach ($fetch_clients->result() as $row) {
                                   echo "<option>0" . $row->phone . "</option"; 
                                  }
                                }
                                ?></option>
                                <option><?php 
                  ...
                  <select name="billingaddress" id="billingaddress" class="form-control col-md-6">
                    <option></option>
                     <option><?php 
                                if($fetch_clients->num_rows()>0){
                                  foreach ($fetch_clients->result() as $row) {
  ...
  <table style="border-collapse: collapse;" border="0" align="center"
   width="50%" id="customersAdd" class="table table-bordered" >
    <tr>
      <td><br>
        <input class="btn btn-primary text" style="color: #000;font-weight: 800" type="button" 
...
<div class="form-row container">
                    ...
                    <div class="col-md-4 form-group">
                      <input type="submit" class="dropbtn1" id="save" value="Save" style="font-weight: 800;margin-top: 20%;margin-left: 20%">
                    </div>
                  </div>
                  <br><br>
            ...
            if(stateID) {
             ...
             if(stateID) {
                $.ajax({
                    ...
                    dataType: "json",
                    success:function(data) {
                        $('select[name="billingaddress"]').empty();
                        $.each(data, function(key, value) {
                            $('select[name="billingaddress"]').append('<option value="'+ value.id +'">'+ value.street +'</option>');
                        });
                    }
                });
            }else{
...
swal({
  ...
  $.ajax({
    data : data,
    type : 'POST',
    url : '<?php echo base_url('user/save_quotation');?>',
    crossOrigin : false,
    dataType:'json',
    success : function(result){
      if(result.quotation == "success"){
        swal('Successfully Saved','','success');
          window.location.href = '<?php echo base_url("user/quotations");?>';
      }else{
