      ...
      ?></option>
                  ...
                  <div class="container">
                   <div class="form-row">
                    ...
                    <input type="date" name="transactiondate" id="transactiondate" class="form-control col-md-6">
                    </div>
                    <div class="col-md-4 form-group">
            ...
            if(stateID) {
                $.ajax({
                    ...
                    success:function(data) {
                        ...
                        $.each(data, function(key, value) {
                            $('select[name="billingaddress"]').append('<option value="'+ value.id +'">'+ value.street  +  value.state +'</option>');
                        });
                    }
                });
            }else{
...
swal({
  ...
  $.ajax({
    data : data,
    type : 'POST',
    url : '<?php echo base_url('user/save_cashtransaction');?>',
    crossOrigin : false,
    dataType:'json',
    success : function(result){
      if(result.invoice == "success"){
        swal('Successfully Saved','','success');
        window.location.href = '<?php echo base_url("user/allcash");?>';
      }else{
